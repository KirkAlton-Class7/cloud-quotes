...
# Wait a moment for the filesystem to settle after git reset
sleep 1

mkdir -p "${DATA_DIR}/images"
rm -rf "${DATA_DIR}/images"/* 2>/dev/null || true

# Copy images.json – if missing, log error but don't exit
if [ -f "$REPO_DIR/images.json" ]; then
    cp -f "$REPO_DIR/images.json" "${DATA_DIR}/images.json"
    log "images.json copied"
else
    log "ERROR: images.json not found in repo root – check repo content"
    # Optionally generate a fallback here if you wish
fi
